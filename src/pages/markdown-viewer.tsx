...
export default function MarkdownViewerTool() {
  ...
  const { viewMode, isFullscreen, zoomLevel } = useMarkdownViewerStore()
  ...
  const content = (
    <div className={`flex flex-col gap-4 ${isFullscreen ? 'h-full' : 'h-[calc(100vh-13rem)]'}`}>
      <MarkdownToolbar textareaRef={textareaRef} value={input} onChange={setInput} />
      
      <div className={`grid ${viewMode === 'split' ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 max-w-4xl mx-auto w-full'} gap-4 md:gap-6 flex-1 min-h-0`}>
        {/* Editor Pane */}
